...
describe("TokenNFT test", function (accounts) {
  ...
  it("token setup", async function () {
    ...
    Token1155 = await ethers.getContractFactory("My1155HybridToken");
  ...
  it("creator can mint 1 token with id=1 for other account", async function () {
    tx = await erc1155Token.connect(creator).mint(other1.address, 1, 1, 0x1112);
    //get event
  ...
  it("creator can mint 10 token with id=2 for other2 account", async function () {
    ...
    expect(await erc1155Token.totalSupply(2)).to.be.equal(BigNumber.from("10"));
  ...
  it("creator can batch mint some token with different ids for other2 account", async function () {
    await expect(
      ...
      .withArgs(creator.address, AddressZero, other2.address, [2, 3], [10, 15]);
    ...
    expect(await erc1155Token.totalSupply(3)).to.be.equal(BigNumber.from("15"));
    expect(await erc1155Token.totalSupply(2)).to.be.equal(BigNumber.from("20"));
